...
def show_engine_c():
    ...
    pos = data.get("engine_c", []) + data.get("value", []);
    data["engine_c"] = pos  # normalize for indexing
    ...
    if wl:
        ...
        for j,s in enumerate(wl):
            ...
            card_html = (
                ...
                f"{render_signals_html(signals, engine="C", stock=s, all_wl=wl, ea_score=ea, all_pos=all_pos_list)}"
