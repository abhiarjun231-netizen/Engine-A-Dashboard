    ...
    if not closed_trades:
        ...
        for t in reversed(closed_trades):
            ...
            closed_html = (
                ...
                f"</div>"
                f"<div style='display:flex;justify-content:space-between;margin-top:6px;font-size:12px;'>"
                f"<div style='color:#94a3b8;'>Entry: ₹{t.get('entry', 0):,.0f}</div>"
                f"<div style='color:#94a3b8;'>Exit: ₹{t.get('exit_price', 0):,.0f}</div>"
                f"<div style='color:{pnl_color};font-weight:700;'>{pnl_sign}₹{pnl:,.0f} ({pnl_sign}{pct}%)</div>"
                f"</div>"
                f"<div style='display:flex;justify-content:space-between;margin-top:4px;font-size:11px;'>"
