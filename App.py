...
.score-card{background:linear-gradient(135deg,#4f46e5 0%,#7c3aed 50%,#a855f7 100%);color:white;padding:28px;border-radius:16px;text-align:center}
...
def log_score_history(score, condition):
    try:
        ...
        history.append({'date': today, 'score': score, 'condition': condition})
        history = history[-90:]
...
def log_portfolio(invested, current, pnl):
    try:
        ...
        today = datetime.datetime.now().strftime('%Y-%m-%d')
        history = [h for h in history if h.get('date') != today]
        history.append({'date': today, 'invested': invested, 'current': current, 'pnl': pnl})
        history = history[-90:]
    ...
    for n, t in tk.items():
        try:
            ...
            if n == 'nifty' and len(p) >= 200:
                ...
                continue
            if n == 'indiavix':
                d['india_vix_auto'] = round(cur, 1)
                continue
            old = float(p.iloc[-22]) if len(p) > 22 else float(p.iloc[0])
            chg = round((cur - old) / old * 100, 1)
            if n == 'inr':
                d['inr'] = cur
                d['inr_dir'] = "Weakening" if chg > 2 else ("Strengthening" if chg < -2 else "Stable")
            elif n == 'us10y':
                d['us10y'] = cur
                d['us10y_dir'] = "Rising" if chg > 2 else ("Falling" if chg < -2 else "Stable")
            elif n == 'brent':
                d['brent'] = cur
                d['crude_dir'] = "Rising" if chg > 2 else ("Falling" if chg < -2 else "Stable")
...
for i, (n, (s, m)) in enumerate(sd.items()):
    with cl[i % 4]:
        ...
        comp = "<div class='metric-card' style='padding:14px;'><div style='font-size:12px;color:#94a3b8;'>" + n + "</div><div style='font-size:22px;font-weight:700;color:" + co + ";margin-top:4px;'>" + str(s) + "/" + str(m) + "</div><div style='height:4px;background:#1e293b;border-radius:2px;margin-top:6px;'><div style='height:100%;width:" + str(p) + "%;background:" + co + ";border-radius:2px;'></div></div></div>"
