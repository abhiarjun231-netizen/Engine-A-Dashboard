"""
test_angel.py — Engine A Live Data Fetcher v1
Fetches live market data from Angel One and saves to CSV.
...
print("=" * 50)
print("ENGINE A — LIVE DATA FETCHER")
print("=" * 50)
...
SYMBOLS = {
    ...
    "India VIX":  ("NSE", "India VIX", "99926037"),
}
...
def fetch_ltp(name, exchange, tradingsymbol, token):
    """Fetch last traded price for one symbol. Returns (price, status)."""
    try:
        data = smart.ltpData(
            exchange=exchange,
...
with open(csv_path, "w", newline="") as f:
    writer = csv.DictWriter(f, fieldnames=["timestamp", "symbol", "price", "status"])
    writer.writeheader()
